...
object RowLevelRuntimeFilterSemiJoinReductionApp {

  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder()
      .appName("Spark 3.3.0. joins").master("local[*]")
      .config("spark.sql.optimizer.runtime.bloomFilter.enabled", false)
      .config("spark.sql.optimizer.runtimeFilter.semiJoinReduction.enabled", true)
      .config("spark.sql.optimizer.runtime.bloomFilter.applicationSideScanSizeThreshold", "1B")
