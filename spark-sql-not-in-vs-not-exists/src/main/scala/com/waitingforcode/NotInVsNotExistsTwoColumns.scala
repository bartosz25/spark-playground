...
object NotInVsNotExistsTwoColumns {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val notInQuery = sparkSession.sql(
      """
        |SELECT id, login, number FROM users WHERE (id, number) NOT IN (SELECT userId, registeredNumber FROM registered)
        |""".stripMargin)
    ...
    val notExistsQuery = sparkSession.sql(
      """
        ...
        |WHERE r.userId <=> id AND r.registeredNumber <=> nu,ber)
        |""".stripMargin)
