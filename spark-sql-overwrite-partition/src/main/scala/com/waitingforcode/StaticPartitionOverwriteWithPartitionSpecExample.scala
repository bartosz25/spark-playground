...
object StaticPartitionOverwriteWithPartitionSpecExample {

  def main(args: Array[String]): Unit = {
    val outputDir = "/tmp/spark-playground/partitions/static-overwrite-partition-spec-example"
    ...
    sparkSession.sql(
      """
        |INSERT OVERWRITE test_table PARTITION(nr=3) (upper_case, lower_case)
        |VALUES
        |("Cc", "cc")
        |""".stripMargin)
    sparkSession.sql("SELECT * FROM test_table").show()
