...
object KafkaChangesDataLossConfigurationDemo extends App {
  ...
  val inputKafkaRecords = sparkSession.readStream
    ...
    .option("startingOffsetsByTimestamp", """{"kafka_changes":{"0":20000}}""")
    // Note: it doesn't apply to the `startingOffsets` option working on the offsets
