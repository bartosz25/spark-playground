...
class FramedSerializer(Serializer):
    ...
    def dump_stream(self, iterator, stream):
        for obj in iterator:
            import logging
            print(f'Dumping stream for {obj}')
...
class BatchedSerializer(Serializer):
    ...
    def dump_stream(self, iterator, stream):
        print(f'Dumping2 stream for {iterator}')
...
class NoOpSerializer(FramedSerializer):
    ...
    def loads(self, obj):
        print(f'loads NoOpSerializer stream for {obj}')
        return obj

    def dumps(self, obj):
        print(f'dumps NoOpSerializer stream for {obj}')
        return obj
...
class PickleSerializer(FramedSerializer):
    ...
    def dumps(self, obj):
        print(f'Dumping4 {obj}')
        return pickle.dumps(obj, pickle_protocol)

    def loads(self, obj, encoding="bytes"):
        print(f'loads PickleSerializer stream for {pickle.loads(obj, encoding=encoding)}')
...
class CloudPickleSerializer(PickleSerializer):
    ...
    def dumps(self, obj):
        print(f'Dumping5 stream for {obj}')
        try:
...
class MarshalSerializer(FramedSerializer):
    ...
    def dumps(self, obj):
        print(f'Dumping6 stream for {iterator}')
        return marshal.dumps(obj)

    def loads(self, obj):
        print(f'loads6 stream for {obj}')
...
class AutoSerializer(FramedSerializer):
    ...
    def dumps(self, obj):
        print(f'Dumping7 stream for {obj}')
    ...
    def loads(self, obj):
        print(f'loads6 AutoSerializer for {obj}')
        _type = obj[0]
        if _type == b'M':
...
class CompressedSerializer(FramedSerializer):
    ...
    def dumps(self, obj):
        print(f'Dumping8 stream for {obj}')
        return zlib.compress(self.serializer.dumps(obj), 1)

    def loads(self, obj):
        print(f'loads Dumping8 stream for {obj}')
...
class UTF8Deserializer(Serializer):
    ...
    def loads(self, stream):
        print(f'loads UTF8Deserializer stream for {stream}')
        length = read_int(stream)
        if length == SpecialLengths.END_OF_DATA_SECTION:
            raise EOFError
        elif length == SpecialLengths.NULL:
        ...
        x = s.decode("utf-8") if self.use_unicode else s
        print(f'Decode = {x}')
        return x
